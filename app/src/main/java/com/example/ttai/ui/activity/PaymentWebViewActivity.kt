...
class PaymentWebViewActivity : AppCompatActivity() {
    ...
    /** 支付宝 WAP 支付完成后的同步跳转地址 */
    private fun isAlipayReturnUrl(url: String): Boolean {
        return url.contains("alipay.trade.wap.pay.return", ignoreCase = true) ||
            (url.contains("becomestar.com.cn", ignoreCase = true) &&
                url.contains("out_trade_no", ignoreCase = true) &&
                url.contains("trade_no", ignoreCase = true))
    }

    private fun handleUrl(url: String) {
        when {
            isAlipayReturnUrl(url) -> {
                handlePaymentSuccess()
            }
    ...
    private fun handlePaymentSuccess() {
        ...
        val resultIntent = Intent().apply {
            putExtra(EXTRA_ORDER_ID, intent.getStringExtra(EXTRA_ORDER_ID))
        }
