 ...
 * 显示商店中可购买的商品
 */
class ShopBeautifyFragment : BaseMviFragment<ShopBeautifyFragmentIntent, ShopBeautifyFragmentState, ShopBeautifyFragmentViewModel, FragmentShopBeautifyBinding>() {
    
    override val viewModel: ShopBeautifyFragmentViewModel by viewModels {
        ShopBeautifyFragmentViewModelFactory(
            requireContext()
        )
    }
    override val binding by viewBinding { inflater, container, attachToParent ->
        FragmentShopBeautifyBinding.inflate(inflater, container, attachToParent)
    }
    ...
    override fun render(state: ShopBeautifyFragmentState) {
        // 更新商品列表
    ...
    private fun performPurchase(itemId: String) {
        sendIntent(ShopBeautifyFragmentIntent.PurchaseItem(itemId, 1))
    }
